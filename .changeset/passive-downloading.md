...
- `progress` is `null` while `status === "downloading"` until the browser actually reports a fraction via a `downloadprogress` event — it no longer fabricates a starting `0`. This applies to every download, not just joined ones: a hook-driven download you triggered yourself now also starts at `null` and moves to a number only once the browser says so. `useGlobalDownloadProgress()`'s aggregate is unaffected — it already coalesces an in-flight download with no signal yet to `0`.
